...
from bop_toolkit_lib import pycococreatortools
...
for scene_id in dp_split['scene_ids']:
    ...
    for scene_view, inst_list in scene_gt.items():
        ...
        # img_path = dp_split['gray_tpath'].format(scene_id=scene_id, im_id=im_id)
        relative_img_path = os.path.relpath(img_path, ROOT_DIR)
        relative_img_path = relative_img_path.split('/', 1)[1]

        image_info = pycococreatortools.create_image_info(image_id, relative_img_path, dp_split['im_size'])
        ...
        for idx, inst in enumerate(inst_list):
            ...
            binary_inst_mask = np.asarray(Image.open(mask_p).convert('1')).astype(np.uint8)
            category_info = {'id': category_info, 'is_crowd': 0}
...
with open('{}/coco_{}_{}.json'.format(ROOT_DIR, dataset_name, split), 'w') as output_json_file:
